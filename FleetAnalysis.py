...
companyName = 'South and Vale District Council'

vehicleIdentifier = 'SURNAME' #This should be the column name of the correct column
...
fleetFile = pd.read_csv(fileLocation,encoding='cp1252', delimiter=',')
...
#dayArray = dayArray.reindex(index=dayArray.index.to_series().str[-2:].astype(int).sort_values().index)
